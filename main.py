...
def add_record(record):
    '''Добавляет запись в глобальный массив записей, сортирует его в порядке убывания
    и удаляет наименьшую запись, чтобы поддерживать фиксированный размер массива.
    
    :param record: Запись, которую нужно добавить. Ожидается, что это 
                   целое число или строка, представляющая целое число.
    :type record: int or str
    :raises ValueError: Если переданное значение не может быть преобразовано в целое число.
    :raises IndexError: Если глобальный массив записей пуст перед вызовом функции.
    '''
    global records_mass
